  ...
  return (
    <div className="bg-slate-800 p-4 rounded-lg shadow-lg overflow-x-auto relative" ref={chartContainerRef}>
      ...
      <div style={{ width: totalUnits * unitWidth + labelWidth + 8, minHeight: (tasks.length + 2) * rowHeight + CHART_PADDING*2 }} className="relative">
        {/* Date Headers and Resizer - This entire block is already sticky for vertical scroll */}
        <div className="sticky top-0 z-20 bg-slate-800 flex" style={{ height: rowHeight }}> {/* Use rowHeight here */}
          {/* Top-left fixed cell (for label header area) */}
          <div style={{ width: labelWidth, flexShrink: 0 }} className="sticky left-0 z-30 bg-slate-800 border-r border-b border-slate-700">
            {/* Can be empty or add a title like "Tasks" */}
          </div>
          {/* Resizer Handle - also needs to be sticky */}
          <div
            ref={resizeHandleRef}
            onMouseDown={handleMouseDown}
            style={{
              left: labelWidth, // Dynamically set left position for stickiness
              width: '8px',
              cursor: 'ew-resize',
              backgroundColor: isResizing ? '#0ea5e9' : '#334155'
              // position: 'sticky' and top: 0 are handled by className
            }}
            className="sticky top-0 h-full flex-shrink-0 border-b border-slate-700 hover:bg-sky-600 transition-colors z-20"
            title="Drag to resize task name column"
          >
            &nbsp;
          </div>
          {/* Date units - these will scroll horizontally under the sticky header */}
          <div className="flex" style={{paddingLeft: 0}}>
            {dateUnits.map((unitDate, i) => {
              ...
              if (timeUnit === 'day') {
                ...
                displayContent = displayDate; // 週表示は変更なし
              } else { // month
                const displayDate = unitDate.toLocaleDateString(undefined, { year: 'numeric', month: 'long' });
                displayContent = displayDate; // 月表示は変更なし
              }
              return (
                <div
                  key={i}
                  style={{ width: unitWidth }}
                  className="flex-shrink-0 border-r border-b border-slate-700 text-xs text-slate-400 flex items-center justify-center px-1 text-center h-full"
                >
                  {displayContent}
                </div>
              );
